...
export const TERMS_PART3: StudyTerm[] = [
  ...
  },
  {
    ...
    simpleExplanation: 'O JWT (JSON Web Token) é um token compacto e assinado. O payload é Base64Url, então ele não é criptografado por padrão. Quando seu usuário faz login, você envia esse token para provar quem ele é nas próximas requisições.',
    interviewExplanation: 'JWT é um padrão compacto e autocontido de segurança (RFC 7519) formado por três blocos separados por pontos: Header, Payload (com claims) e Signature. A assinatura protege a integridade do token; o conteúdo não fica criptografado por padrão.',
    ...
    shortInterviewAnswer: 'Utiliza uma chave secreta no servidor C# para assinar o token. Se alguém alterar os dados por fora, a assinatura não bate e o servidor recusa a conexão imediatamente.',
    betterInterviewAnswer: 'O token JWT é legível até a parte do Payload. A segurança está na Signature. O servidor C# usa algoritmos como HMAC-SHA256 com uma chave secreta para assinar o conteúdo anterior do token. Se um atacante alterar um caractere do payload, a assinatura falhará no backend.',
  ...
  },
  {
    ...
    whenToUse: 'Sempre que você precisar carregar pequenos dados de identidade para regras de autorização e exibição.',
    ...
    betterInterviewAnswer: 'Permite autorizar de forma granular examinando os claims contidos na identidade do usuário. Exemplo: podemos criar uma policy em Program.cs que concede acesso para um endpoint apenas para usuários cujo claim "Idade" seja superior a 18 anos.',
  ...
  },
  {
    ...
    whenToUse: 'Dedicado a sistemas com agrupamentos hierárquicos simples e de fácil manutenção estrutural.',
    ...
    shortInterviewAnswer: 'Roles são uma forma mais simples e mais rígida de autorização. Policies costumam ser mais flexíveis e centralizadas.',
    betterInterviewAnswer: 'Roles funcionam bem para grupos simples como Admin ou Gerente, mas podem espalhar regras de autorização pelos atributos. Policies centralizam as regras no Program.cs e são melhores quando o acesso depende de mais de uma condição.',
  ...
  },
  {
    ...
    interviewExplanation: 'O padrão Repository encapsula a leitura e gravação em fontes persistentes, fornecendo uma fachada para o domínio consumir dados sem conhecer detalhes de acesso.',
    ...
    shortInterviewAnswer: 'Ele pode simplificar testes e isolar detalhes de persistência, mas vira repetição se só copiar chamadas simples do EF Core.',
    betterInterviewAnswer: 'O Repository faz mais sentido quando há regras ou isolamento real na persistência. Em projetos simples, o DbContext e o DbSet já cobrem grande parte do fluxo, então um repositório extra só vale quando adiciona valor de verdade ao domínio.',
  ...
  },
  {
    ...
    simpleExplanation: 'SOLID são 5 princípios de design pensados para tornar o código mais legível, testável e fácil de evoluir sem quebrar o que já existe.',
    ...
    commonErrors: ['Levar os princípios ao extremo e criar abstrações desnecessárias, microserviços vazios ou camadas sem utilidade prática.'],
    ...
    betterInterviewAnswer: 'A Inversão de Dependência (DIP) garante flexibilidade e substituição limpa ao longo de todo o sistema C#. Lógicas de alto nível não devem acoplar instâncias diretamente com detalhes de infraestrutura, como drivers de banco. No lugar, declaramos e injetamos abstrações e interfaces que definem os contratos das operações. Isso torna tudo mais testável com dublês e menos preso a detalhes concretos.',
  ...
  },
  {
    ...
    simpleExplanation: 'O Docker empacota o seu aplicativo .NET e suas dependências dentro de um contêiner isolado. O contêiner compartilha o kernel do host e ajuda a evitar o clássico "na minha máquina funcionava".',
