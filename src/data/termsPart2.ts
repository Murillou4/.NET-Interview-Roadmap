...
export const TERMS_PART2: StudyTerm[] = [
  ...
  },
  {
    ...
    commonErrors: ['Usar entidades diretas de banco de dados (como as do EF Core) como parâmetros de rota de API, o que expõe conexões sensíveis de relacionamento e expõe dados internos do seu banco de dados.'],
  ...
  },
  {
    ...
    interviewExplanation: 'Consiste na checagem sistemática de regras do contrato e domínio nos payloads de entrada do sistema. No .NET Core, é implementado via Data Annotations simplificados ou com o pacote profissional FluentValidation.',
    ...
    betterInterviewAnswer: 'FluorValidation é uma biblioteca que remove o acoplamento do Data Annotations que decora propriedades com tags confusas. Ele permite criar classes validadoras isoladas usando Fluent API robusta, viabilizando a validação de regras de banco integradas, mensagens customizadas de acordo com condições e testes unitários de regras isoladas com facilidade.',
  ...
  },
  {
    ...
    commonErrors: ['Instutir um Transient que carrega buffers gigantescos de cache interno nas propriedades, gastando ciclos e memória RAM à toa.'],
  ...
  },
  {
    ...
    practicalExample: 'builder.Services.AddScoped<IPeiddoRepository, PedidoRepository>();',
  ...
  },
  {
    ...
    interviewExplanation: 'A classe DbContext é o ponto de contato principal do Entity Framework Core. Ela atua como uma combinação dos padrões de design Repository e Unit of Work, permitindo consultar e agrupar mutações coletivas sobre o banco.',
    ...
    shortInterviewAnswer: 'Ele agrupa todas as alterações feitas em memória pelas suas entidades no mesmo request e só as grava no banco físico juntas quando você chama o comando .SaveChangesAsync().',
    betterInterviewAnswer: 'DbContext implementa o padrão Unit of Work. Significa que ao criarmos, editarmos ou removermos múltiplos objetos que dependem de tabelas diferentes, todas as modificações são agrupadas em rastreamento síncrono interno. Ao chamar .SaveChangesAsync(), todas são executadas no banco de forma transacionada e lógica, reduzindo acessos repetidos e isolando a consistência.',
  ...
  },
  {
    ...
    level: 'intermediário',
    category: 'Entity Framework Core',
    simpleExplanation: 'O Include serve para carregar relacionamentos. Se o "Cliente" se liga a uma lista de "Pedidos", o EF Core por padrão não traz os pedidos para economizar memória (Lazy Loading desativado). Se você quer os pedidos juntos na busca, você usa o Include para carregar tudo no mesmo SQL.',
    interviewExplanation: 'Consiste no padrão de carregamento ganancioso (Eager Loading) nativo do EF Core. Ele gera queries do SQL usando comandos JOIN de forma que o banco retorne a entidade principal combinada com suas propriedades e tabelas dependentes colecionadoras.',
    ...
    commonErrors: ['Abusar do Include aninhando 10 tabelas pesadas (Include seguido de vários ThenInclude). Isso gera uma explosão cartesiana monumental de registros repetidos, deteriorando totalmente a performance do servidor SQL.'],
    interviewQuestion: 'O que o método Include realiza de forma prática sob o capô?',
    shortInterviewAnswer: 'Utiliza instruções JOIN no banco de dados relacional para instanciar propriedades complexas da classe no mesmo comando de busca.',
    betterInterviewAnswer: 'O Include parametriza Eager Loading. Em bancos SQL, ele altera a consulta traduzindo-a via INNER JOIN ou LEFT JOIN dependendo se o relacionamento é obrigatório ou opcional. Devemos utilizá-lo de forma moderada, ou combiná-lo com splits de queries (.AsSplitQuery()) para contornar explosões combinatórias no banco.',
    tags: ['EF Core', 'JOINS', 'Performance'],
    quiz: {
      question: 'O que acontece por padrão se você buscar um Cliente e tentar acessar sua lista de Pedidos sem ter usado o modificador "Include"?',
      options: ['O C# calcula de forma estática', 'A propriedade correspondente virá nula (null), o que pode disparar uma exceção de nulo se você tentar acessá-la imediatamente', 'Os dados são baixados via Bluetooth', 'Roda um prompt pedindo ajuda ao usuário'],
      answerIndex: 1,
      explanation: 'Sem Eager Loading (Include), propriedades de referências complexas não mapeadas na memória local do tracker do EF virão nulas.'
    }
  },
  {
    id: 'asnotracking',
    ...
    level: 'avançado',
    category: 'Entity Framework Core',
    simpleExplanation: 'O AsNoTracking diz ao C#: "Busque essas informações no banco rápidas, mas não gaste memória rastreando se eu vou editá-las depois. É só para ler". Isso economiza RAM do servidor e deixa a pesquisa muito mais veloz.',
    interviewExplanation: 'AsNoTracking instrui o EF Core a ignorar o mecanismo de rastreamento de alterações (Change Tracker) para os registros retornados. Ideal para rotas focadas em leitura.',
    ...
    shortInterviewAnswer: 'Porque desliga a cópia de segurança em memória de rastreamento de alteração de cada entidade, reduzindo uso de RAM e simplificando os ciclos da CPU do servidor.',
    betterInterviewAnswer: 'Ao usar consultas normais, o EF Core aloca um clone oculto na memória RAM para gerenciar diferenças no momento de fazer o Save. O uso de .AsNoTracking() instrui o EF a ignorar a alocação dessas cópias e pular a inserção do item no Change Tracker. Isso economiza grandes fracos de alocações RAM e simplifica a renderização interna de dados da API.',
  ...
  },
  {
    ...
    interviewExplanation: 'O problema N+1 clássico no ecossistema .NET decorre do uso de Lazy Loading nas queries de coleções onde o compilador envia implicitamente 1 query inicial para reter a lista principal de tamanho N e, sequencialmente, o laço de iteração dispara sub-queries individuais, degradando severamente a performance física.',
