...
export const TERMS_PART1: StudyTerm[] = [
  ...
  },
  {
    ...
    level: 'intermediário',
    category: 'C# básico/intermediário',
    simpleExplanation: 'Struct é um tipo de valor (Value Type). Diferente de uma classe, a struct guarda os dados reais direto na memória rápida (Stack). Se você passar uma struct para uma função, o C# faz uma cópia inteira dela na memória.',
    interviewExplanation: 'Uma struct no C# é um Value Type ideal para estruturas de dados pequenas e imutáveis. Ela não suporta herança (exceto implementar interfaces) e perde performance se contiver muitos campos, devido ao custo de cópia de valor nas atribuições.',
  ...
  },
  {
    ...
    interviewExplanation: 'A List<T> é uma coleção genérica indexável alocada sequencialmente na memória Heap. Ela encapsula um array tradicional interno e gerencia o redimensionamento dinâmico (duplicando a capacidade quando chega ao limite).',
    ...
    shortInterviewAnswer: 'Internamente há um array fixo. Quando ele enche, a classe aloca um novo de dobro do tamanho e copia todos os itens.',
    betterInterviewAnswer: 'A capacidade padrão inicial é definida. Ao atingir o limite de armazenamento, ela faz uma "rehashing", criando um novo array com capacidade dobrada (Capacity * 2), copiando os dados anteriores. Métodos como Add() operam em O(1) amortizado devido a essa estratégia.',
  ...
  },
  {
    ...
    level: 'avançado',
    category: 'Coleções e LINQ',
    simpleExplanation: 'O IQueryable é uma lista feita para bancos de dados. Quando você faz filtros ou buscas com ele, ele não traz tudo para a memória. Ele traduz seus filtros para comandos SQL reais de forma que a filtragem ocorra no próprio banco de dados.',
    interviewExplanation: 'IQueryable<T> estende IEnumerable e é focado no processamento de Queries "Out-of-Memory". Ele carrega uma Expression Tree (árvore de expressões) que provedores ORM como EF Core traduzem para sintaxe SQL proprietária.',
  ...
  },
  {
    ...
    interviewExplanation: 'A estrutura de tratamento estruturado de exceções (Structured Exception Handling) do C# avalia um bloco vigiado (try) e direciona uma Exception lançada para um ou mais tratadores compatíveis (catch), com a possibilidade de varrer recursos limpos com a chave opcional dynamic "finally".',
  ...
  },
  {
    ...
    interviewExplanation: 'Em C#, todas as exceções derivam da classe base System.Exception. Elas representam condições extraordinárias de desvio do fluxo regular do sistema e carregam metadados preciosos para telemetria de erros (Mensagem, StackTrace, e InnerExceptions).',
